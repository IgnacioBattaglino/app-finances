...
- **Movimientos / transactions**: `pages/Movements.jsx` (mezcla transactions y contributions, incluidas las cuentas de ahorro) · `components/TransactionFormModal.jsx` · `lib/transactions.js`, `lib/movements.js` (`monthTotals`: los cinco renglones del período, y el único lugar que decide qué es Invertido y qué es Ahorrado) · `lib/movementList.js` (cómo se arma la LISTA: el apareo de las dos patas de una transferencia y de los repartos de un conteo en una sola línea, y en qué cajón cae cada una) · `components/form/FilterChips.jsx` (los seis filtros) · `lib/dateRange.js` + `components/movements/RangeSheet.jsx` (qué período se mira: mes, año, todo o dos fechas) · `lib/pagination.js` (el corte silencioso de PostgREST en 1000 filas, en un solo lugar).
...
- **Cálculo del disponible**: `lib/liquid.js` (líquido, desglose por cuenta, reconciliación, `planReconciliation`: el neteo de un conteo —la misma regla que aplica la base al escribir— y el borrado de un conteo entero) · `components/LiquidModal.jsx` (modal, se abre desde Dashboard).
