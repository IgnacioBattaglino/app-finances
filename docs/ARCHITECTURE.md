...
| is_savings | boolean NOT NULL default false | (migración 0036) la plata guardada, que no es la del día a día. Las cuentas anteriores quedan en false. Hoy el dato se guarda y `get_liquid_by_account` lo devuelve, pero **ninguna pantalla lo mira todavía**: es el insumo de los cuatro números (disponible / ahorro / invertido / total) de un paso posterior |
| created_at | timestamptz default now() | |
