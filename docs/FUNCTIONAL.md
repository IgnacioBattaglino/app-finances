...
- ✅ Bolsas de activos personalizables por usuario (reemplazan a los 5 tipos fijos anteriores): cada bolsa tiene un nombre libre y tres flags que definen su comportamiento — modo de valuación (vale lo aportado sin pedir carga nunca / carga manual periódica / precio en vivo con identificador por activo), si busca rendimiento por default, y si suma al valor total del portafolio. Cada usuario arranca con 5 bolsas sembradas (Cripto, CEDEARs, Renta fija, Fondos, Efectivo USD) que reproducen el comportamiento anterior, pero son solo un punto de partida: se pueden crear, renombrar y archivar/eliminar bolsas propias desde el modal de activo ("+ Nueva bolsa", ícono de lápiz para renombrar, "Gestionar bolsa" para archivar/eliminar).
...
- ✅ Alta de aportes: fecha, activo, monto (en ARS convertido al MEP del día, o directo en USD); si la bolsa del activo es de precio en vivo, además la cantidad comprada (ej: 0.001 BTC), obligatoria. Marca "ya lo tenía" para tenencias preexistentes que no descuentan del líquido.
- ✅ Valor actual según el modo de valuación de la bolsa: automático para precio en vivo con identificador (cantidad acumulada × precio, con caída al último valor manual si la API falla o falta el identificador — hoy solo cripto vía CoinGecko resuelve precio real); carga manual periódica para bolsas de valuación manual; vale lo aportado sin pedir carga para bolsas de ese modo (hoy Efectivo USD).
