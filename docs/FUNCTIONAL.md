...
- ✅ Gestión de categorías (crear, renombrar, archivar, restaurar). El alta está al pie del grupo al que va a pertenecer, así que ya no hace falta un segmentado Gasto/Ingreso adentro del form: el lugar dice el tipo. El detalle de una categoría muestra su tipo (antes, editando, no había forma de saber si estabas tocando un gasto o un ingreso) y archivar pide confirmación suave, como manda la convención.
- ✅ Las categorías del sistema ("Ajuste de saldo", gasto e ingreso) se muestran con un badge y su detalle es de solo lectura: no se pueden renombrar ni archivar.
