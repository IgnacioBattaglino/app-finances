...
export function SettingsGroup({ title, footer, children }) {
  return (
    <section>
      {title && (
        <h2 className="mb-1.5 px-4 text-[11px] font-semibold uppercase tracking-[0.08em] text-ink-soft">
          {title}
        </h2>
      )}
      ...
      {footer && <p className="mt-1.5 px-4 text-xs text-ink-soft">{footer}</p>}
    </section>
  )
}
...
export function SettingsLinkRow({ to, label, value, badge }) {
  return (
    <Link
      to={to}
      className="flex w-full items-center gap-3 px-4 py-3 text-left transition hover:bg-mist/50 active:bg-mist/60"
    >
      <span className="flex min-w-0 flex-1 items-center gap-1.5">
        <span className="truncate text-[15px]">{label}</span>
        {badge && (
          <span className="shrink-0 rounded-full bg-mist px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-ink-soft">
            {badge}
          </span>
        )}
      </span>
      {value && <span className="shrink-0 text-[15px] text-ink-soft">{value}</span>}
      <Chevron />
    </Link>
  )
}
...
export function SettingsValueRow({ label, value }) {
  return (
    <div className="flex items-center justify-between gap-3 px-4 py-3">
      ...
      <span className="truncate text-[15px] text-ink-soft">{value}</span>
    </div>
  )
}
...
export function SettingsButtonRow({ onClick, label, tone = 'accent', disabled = false }) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`w-full px-4 py-3 text-left text-[15px] font-medium transition active:bg-mist/60 disabled:opacity-40 ${color}`}
    >
      {label}
    </button>
  )
